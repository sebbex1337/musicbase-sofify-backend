...
albumsRouter.delete("/:id", async (req, res) => {
    ...
    DELETE FROM artists_albums WHERE albumID = ?;`;
    const queryTracksAlbums = /*sql*/ `
	DELETE FROM tracks_albums WHERE albumID = ?;`;
    const value = [id];

    const [artistsAlbumsResults] = await connection.execute(queryArtistsAlbums, value);
    const [tracksAlbumsResults] = await connection.execute(queryTracksAlbums, value);
...
export function prepareAlbumsData(results) {
